...
def create_task():
    ...
    try:
        ...
        due_date = None
        if data.get('date'):
            due_date = datetime.fromisoformat(data['date'] + 'T00:00:00')
        ...
        task = Task(
            ...
            due_date=due_date,
            tags=tags,
            estimated_time=data.get('duration'),
            xp_reward=xp_reward
        )
        db.session.add(task)
        db.session.commit()
...
def update_task(task_id):
    ...
    try:
        ...
        task.estimated_time = data.get('duration', task.estimated_time)
        ...
        # Parse due date
        if data.get('date'):
            task.due_date = datetime.fromisoformat(data['date'] + 'T00:00:00')
...
@tasks_bp.route('/tasks/<int:task_id>/subtasks/<int:subtask_id>/toggle', methods=['PATCH'])
@jwt_required()
...
@tasks_bp.route('/tasks/<int:task_id>/subtasks/<int:subtask_id>', methods=['DELETE'])
@jwt_required()
